...
pub unsafe fn step_idle_orders() {
            ...
            if o.user.health() < o.panic_health {
                for decl in deathrattles {
                    if decl.unit_valid(&o.user, &[], CheckTargetingFlags::Yes, game) {
                        ...
                        let panic_target = find_idle_order_target(&o.user, &decl, &[], &ctx);
    ...
    for &mut (ref decl, ref mut state) in orders.orders.iter_mut().rev() {
        if state.next_frame <= current_frame {
            if let Some((user, target)) = find_idle_order_user_target_pair(decl, &ongoing, game) {
...
fn find_idle_order_user_target_pair(
    decl: &IdleOrder,
    ongoing: &[OngoingOrder],
    game: Game,
) -> Option<(Unit, Unit)> {
    fn find_normal(
        ...
        let (target, distance) = find_idle_order_target(&unit, decl, &ongoing, ctx)?;
        let (user, distance) = {
            if distance > 16 * 32 || distance > decl.radius as u32 {
                match find_idle_order_user(&target, decl, &ongoing, ctx.game) {
                    None => (unit, distance),
                    Some(s) => s,
                }
            } else {
