...
pub struct Game {
    ...
    pub leaderboard_location: u8,
    pub leaderboard_parameter: u16,
    pub leaderboard_goal: u32,
    pub leaderboard_string: u32,
    ...
    pub tech_in_progress: [[u8; 0x6]; 0xc],
    pub upgrade_limit_bw: [[u8; 0xf]; 0xc],
    pub upgrade_level_bw: [[u8; 0xf]; 0xc],
    pub upgrade_in_progress: [[u8; 0x8]; 0xc],
    pub is_bw: u8,
    pub unk10351: u8,
...
pub struct AiTown {
    pub next: *mut AiTown,
    pub prev: *mut AiTown,
    pub workers: AiListHead<1000, WorkerAi>,
    pub buildings: AiListHead<1000, BuildingAi>,
    pub player: u8,
    pub inited: u8,
    pub worker_count: u8,
    pub worker_limit: u8,
...
pub struct AiRegion {
    ...
    pub military: AiListHead<1000, MilitaryAi>,
}
...
#[repr(C)]
pub struct AiListHead<const N: usize, T> {
    pub full_array: *mut AiArray<N, T>,
    pub first: *mut T,
}

#[repr(C)]
pub struct AiArray<const N: usize, T> {
    pub ais: [T; N],
    pub first_free: *mut T,
}
...
pub struct PlayerAiData {
    ...
    pub defense_units: [[u16; 0x14]; 8],
    pub build_limits: [u8; 0xe4],
    pub free_medic: *mut Unit,
    pub gencmd_area: Rect32,
}
...
pub struct ResourceAreaArray {
    // Entry 0 and 249 are always unused.
    pub areas: [ResourceArea; 250],
    pub used_count: u32,
...
mod test {
    ...
    fn test_sizes() {
        ...
        assert_eq!(mem::size_of::<ResourceAreaArray>(), 0x2ee8);
        assert_eq!(mem::size_of::<ResourceArea>(), 0x30);
